...
Source: https://github.com/zizheng-guo/RhythmFormer/blob/main/neural_methods/model/base/video_bra.py
"""
import torch
...
def _grid2seq(x: Tensor, region_size: Tuple[int], num_heads: int):
    ...
    x = x.view(B, num_heads, C // num_heads, region_t, region_size[0], region_h, region_size[1], region_w,
               region_size[2])
    x = torch.einsum('bmdtohpwq->bmthwopqd', x).flatten(2, 4).flatten(-4,
...
def _seq2grid(x: Tensor, region_t: int, region_h: int, region_w: int, region_size: Tuple[int]):
    ...
    x = torch.einsum('bmthwopqd->bmdtohpwq', x).reshape(bs, nhead * head_dim,
                                                        region_t * region_size[0], region_h * region_size[1],
                                                        region_w * region_size[2])
    return x
